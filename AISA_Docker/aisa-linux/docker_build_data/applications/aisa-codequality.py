...
def rename_html_report(input_folder, CI):
    files = []
    print(f'INFO - Try to rename html report to "aie_report.html"')
    try:
        dir_files = os.listdir(input_folder)
        for f in [os.path.join(input_folder, basename) for basename in dir_files]:
            if '.html' in f:
                files.append(f)
        if len(files) > 0:
            file = max(files, key=os.path.getctime)
            old_file = os.path.join(file)
            CI.html_report = os.path.join(input_folder, "aie_report.html")
            os.rename(old_file, CI.html_report)
            print(f'INFO - {CI.html_report} was renamed successfully')
        else:
            print('WARNING - AI html-report not found.')
            CI.html_report = None

    except IndexError as e:
        try:
            print(f'\nWARNING - Report in {input_folder} directory not found.')
        except KeyError as ke:
            print('\nWARNING - Report directory not found')
...
def convert_to_markdown(items_total, items_info, items_minor, items_major, items_critical, items_blocker, CI,
    ...
    if CI.html_report is not None:
        if CI.ci_name == 'GitLab':
            ...
            CI.gitlab_artifacts_url = f'{gitlab_artifacts_url}/artifacts/file/{input_folder}/aie_report.html'
            ...
            CI.tc_artifacts_url = f'{CI.teamcity_server_url}/repository/download/{CI.teamcity_buildType_id}/' \
                                  f'{CI.teamcity_build_id}:id/aie_report.html'
...
def main(input_folder, output_file, gitlab_token, settings_file, block_mr, income_args):
    ...
    rename_html_report(input_folder, CI)
